/**
 * accedeweb-tablist - WAI-ARIA tablist plugin based on AcceDe Web instructions
 * @version v0.0.0
 * @link http://www.accede-web.com/
 * @license ISC
 */
...
(function(window, document, exportName) {
  ...
  var Tablist = function( elm, options ){
     ...
     */
    this.getOpenedTabs = function(){
      return tablist.openedTab.slice();
    };
    ...
    this.setTablist = function setTablist(){
      ...
      Array.prototype.forEach.call( tablist.querySelectorAll( '[role=tab]' ), ( function( tab ){
        ...
        if( tab.hasAttribute( 'data-tab-open' ) && !tab.disabled ){
          if( (!tablist.multiselectable && !tablist.openedTab.length) || tablist.multiselectable ){
      ...
      if( tablist.openedTab.length ){
        tablist.openedTab[0].setAttribute( 'tabindex', 0 );
      }
      // if there's no opened tab and it's not an accordion open the first tab
      else if( !tablist.openedTab.length && null === tablist.multiselectable ){
        firstTab.setAttribute( 'tabindex', 0 );
        firstTab.setAttribute( 'aria-expanded', true );
        firstTab.tabPanel.setAttribute( 'aria-hidden', false );
        tablist.openedTab.push( firstTab );
      }
      else{
        firstTab.setAttribute( 'tabindex', 0 );
      }
    ...
    this.switchTab = function switchTab( index ){
      ...
      // handle disabled tab
      var newIndex;
      if ( tablist.tabs[ index ] && tablist.tabs[ index ].disabled) {
